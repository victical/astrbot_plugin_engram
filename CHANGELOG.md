...
- 路由瘦身（第二阶段）：`main.py` 的 `mem_*`、`profile_*`、`engram_force_*`、`rebuild_vector` 指令逻辑下沉到 handlers，`on_private_message` 的 OneBot 同步改为委托 `OneBotSyncHandler`，并新增 `MemoryToolHandler` 承接工具检索输出构建。

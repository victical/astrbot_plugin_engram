...
4. **独立归档节奏**：群聊可单独配置归档静默时间与最少消息数，不必强行复用私聊策略
5. **可配置归属**：支持按群或按人归档群聊记忆
6. **可控联动**：支持在群聊中可选召回私聊记忆，但默认关闭
7. **复用主能力**：尽量复用 `MemoryManager`、`MemoryScheduler`、`MemoryCommandHandler`，降低维护成本
...
| `group_memory_timeout` | `600` | 群聊静默多久后允许触发归档 |
| `group_min_msg_count` | `6` | 群聊未归档消息达到多少条才触发归档 |
...
### 7.2.4 `group_memory_timeout`

用于控制群聊在“最后一次有效交互后静默多久”才触发归档。

- 已启用群聊记忆时，后台调度器会优先读取该值
- 若未配置，则回退到私聊的 `private_memory_timeout`
...
### 7.2.5 `group_min_msg_count`

用于控制群聊至少积累多少条未归档消息才触发归档。

- 已启用群聊记忆时，后台调度器会优先读取该值
- 若未配置，则回退到私聊的 `min_msg_count`

这样可以避免群聊因为碎片消息过多而过早生成质量较差的总结。

### 7.2.6 `group_memory_source_type`
...
### 7.2.7 `group_memory_store_session_as`
...
|---|---|
| `/查看群记忆 [数量]` | 查看本群最近记忆 |
| `/查看群记忆详情 <序号或ID>` | 查看本群记忆详情 |
| `/搜索群记忆 <关键词>` | 搜索本群长期记忆 |
| `/删除群记忆 <序号或ID>` | 删除本群总结记忆 |
| `/删除全部群记忆 <序号或ID>` | 删除本群总结记忆及原始消息 |
| `/撤销删除群记忆` | 撤销本群最近一次删除 |
| `/归档群记忆` | 管理员强制归档本群未处理对话 |
| `/归档全部记忆` | 管理员强制归档全部私聊记忆；若群聊记忆已启用，也会一并归档全部群聊记忆 |
...
5. 用 `_rewrite_group_command_hints()` 把提示文案中的私聊记忆指令替换成群聊记忆指令
...
撤销历史已持久化到 SQLite `deletehistory`，因此：

- 可跨重启恢复
- 每个作用域默认保留最近 3 次
- 群聊与私聊仍按各自数据库/作用域隔离管理

...
当 `enable_profile_affinity = false` 时，`LLMContextInjector` 会从注入文本中移除羁绊相关内容：

- 用户档案块不再包含 `当前羁绊`
- 群聊中被 @ 用户的精简画像不再附加 `relationship_status`
...
- 动态计算画布高度
- 按类别展示标签
- 在 `enable_profile_affinity` 开启时，结合 `BondCalculator` 显示羁绊等级
- 可选附加证据摘要
...
羁绊系统由配置项 `enable_profile_affinity` 控制，默认开启。

开启时，渲染器内部通过 `BondCalculator` 计算：
...
关闭时，`/查看画像` 不再渲染羁绊区域，LLM 注入提示词不再包含羁绊/关系状态。供外部 `astrbot_plugin_affinity` 读取画像与记忆数据的只读 `affinity_memory_provider` 适配器则**始终初始化**，不受本开关影响。

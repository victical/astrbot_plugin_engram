...
4. **内存态缓存**：存储会话计数、近期活动等临时数据（撤销历史与向量补偿任务已持久化到 SQLite）
...
当前 SQLite 包含以下核心表（私聊/群聊库同构）：

1. `rawmemory`：原始消息表
2. `memoryindex`：长期记忆索引表
3. `deletehistory`：删除撤销历史（支持跨重启恢复）
4. `pendingvectorjob`：向量补偿任务队列（失败重试）

此外还包含检索辅助虚表：

- `memoryindex_fts`：SQLite FTS5 虚表（`summary` 全文索引，BM25 候选召回）

> 注：实际 Peewee 默认表名为模型名小写形式；FTS 表与触发器由初始化逻辑自动创建。
...
1. 找到目标 `MemoryIndex`
2. 备份摘要、向量、原始消息 UUID 等到 SQLite `deletehistory`
3. 删除 Chroma 向量
4. 删除或恢复原始消息归档状态
...
- `user_id = ?`
- `memoryindex_fts MATCH ?`（FTS5）
- 可选时间范围、来源类型过滤

说明：

- 默认优先使用 FTS5 + `bm25()` 排序获取候选
- FTS 不可用或关闭时回退到 `summary contains`（LIKE）
- 高选择性过滤仍依赖 `user_id`、`created_at`、`source_type`
...
- 检索时即使向量失效，也可降级到 SQLite FTS5 BM25（失败再回退 LIKE）关键词检索
...
- 未对 JSON 画像写入做文件锁
- 撤销历史已持久化到 `deletehistory`；仍可继续增强审计字段与可视化
...
### 13.2.4 删除撤销持久化已实现（仍可增强审计）

当前撤销历史已落库，支持跨重启恢复；后续可继续补充更细粒度审计字段（操作者、来源端、恢复链路等）。
